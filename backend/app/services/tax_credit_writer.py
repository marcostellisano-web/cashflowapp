...
def _write_breakout_budget(ws, budget: ParsedBudget) -> None:
    ...
    for col in range(1, num_cols + 1):
        ws.cell(row=2, column=col).fill = _GRAND_TOTAL_FILL
        ws.cell(row=3, column=col).fill = _TOTAL_FILL

    # Row 2 — TOTAL: mirror of grand total row
    ws.merge_cells(start_row=2, start_column=1, end_row=2, end_column=6)
    lbl2 = ws.cell(row=2, column=1, value="TOTAL")
    lbl2.font = _WHITE_BOLD
    lbl2.alignment = _LEFT
    lbl2.fill = _GRAND_TOTAL_FILL
    ...
    for col in accounting_cols:
        ...
        c.number_format = _PERCENTAGE_FORMAT
    for col in range(1, num_cols + 1):
        ws.cell(row=3, column=col).border = _THIN_BORDER
