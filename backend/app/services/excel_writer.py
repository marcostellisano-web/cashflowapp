...
def _apply_requested_cashflow_formatting(
    ...
    *,
    ...
    total_fill_max_cols: dict[int, int] | None = None,
) -> None:
    """Normalize sheet formatting to requested style for Cashflow outputs."""
    total_fill_max_cols = total_fill_max_cols or {}
    for row in range(1, ws.max_row + 1):
        for col in range(1, max_col + 1):
            ...
            if row in totals_rows:
                fill_limit_col = total_fill_max_cols.get(row, max_col)
                if col <= fill_limit_col:
...
def _write_main_sheet(wb: Workbook, output: CashflowOutput, params: ProductionParameters):
    ...
    _apply_requested_cashflow_formatting(
        ...
        total_fill_max_cols={fin_total_row: TOTAL_COL},
    )
...
def _write_summary_cf_sheet(wb: Workbook, output: CashflowOutput, params: ProductionParameters) -> None:
    ...
    _apply_requested_cashflow_formatting(
        ...
        keep_paycycle_colors=has_payroll,
        total_fill_max_cols={sum_fin_total_row: TOTAL_COL},
    )
